const fs = require('fs');
let text = fs.readFileSync('src/pages/Services.tsx', 'utf-8');

const newLayers =           <SystemLayerDetail
            number="01"
            title="The Offer."
...
text = text.replace(/<SystemLayerDetail\s+number="01"[\s\S]*?(?=<SystemLayerDetail\s+number="03")/, newLayers);
fs.writeFileSync('src/pages/Services.tsx', text);
console.log("Done");
